...
class Vector:
    ...
    def __add__(self, other):
        ...
        # (1, 2, 3) + (1, 1, 2) = (2, 3, 5)
        return tuple(map(lambda x, y: x + y, self.coords, other.coords))
    
    def __eq__(self, other):
        # two vectors compare == if the have the same len() and the same length()
        return self.length() == other.length() and len(self.coords) == len(other.coords)
    ...
    def __mul__(self, other):
        ...
        #(1, 2, 3) * 2 = (2, 4, 6)
        return tuple(map(lambda x: x * other, self.coords))
